...
import CreateCard from "~/components/administration/userWorkplace/create.vue";
...
export default Vue.extend({
	...
	computed: {
		block() {
			return this.$store.getters["menu/getBlockByName"](
				"administration.createUserWorkplace"
			);
		},
		pageTitle() {
	...
	methods: {
		successedSaved(data) {
			this.$router.replace(`/administration/userWorkplace/${data.id}`);
		}
	}
});
</script>