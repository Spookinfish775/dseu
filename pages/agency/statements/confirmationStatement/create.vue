...
import CreateCard from "~/components/agency/statements/confirmationStatement/create.vue";
...
export default Vue.extend({
	...
	computed: {
		block() {
			return this.$store.getters["menu/getBlockByName"](
				"agency.createConfirmationStatement"
			);
		},
		pageTitle() {
	...
	methods: {
		successedSaved(statement) {
			this.$router.replace(
				`/agency/statements/confirmationStatement/${statement.id}`
			);
		}
	}
});
