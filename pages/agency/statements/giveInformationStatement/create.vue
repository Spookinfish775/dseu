...
import CreateCard from "~/components/agency/statements/giveInformationStatement/create.vue";
...
export default Vue.extend({
	...
	computed: {
		block() {
			return this.$store.getters["menu/getBlockByName"](
				"agency.createGiveInformationStatement"
			);
		},
		pageTitle() {
	...
	methods: {
		successedSaved(statement) {
			this.$router.replace(
				`/agency/statements/giveInformationStatement/${statement.id}`
			);
		}
	}
});
