...
import Card from "~/components/agency/notification/card.vue";
...
export default Vue.extend({
	...
	computed: {
		block() {
			return this.$store.getters["menu/getBlockByName"]("agency.notification");
		},
		pageTitle(): string {
	...
	async asyncData({ $axios, params }) {
		const { data } = await $axios.get(`${dataApi.notification}/${+params.id}`);
		return {
			currentValue: data
		};
	},
