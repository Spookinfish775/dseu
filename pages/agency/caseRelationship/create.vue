...
import CreateCard from "~/components/agency/caseRelationship/create.vue";
...
export default Vue.extend({
	...
	computed: {
		block() {
			return this.$store.getters["menu/getBlockByName"]("agency.createСaseRelationship");
		},
		pageTitle() {
	...
	methods: {
		successedSaved(value) {
			this.$router.replace(`/agency/caseRelationship/${value.id}`);
		}
	}
});
</script>