<template>
  <div class="card-wrapper">
    <BaseToolbar
      ...
      @changeStatus="changeRealEstateStatus"
    />
    <DxForm
      ref="form"
      label-location="top"
    ...
    changeRealEstateStatus() {
      ...
      result.then((dialogResult) => {
        if (dialogResult) {
          this.$awn.asyncBlock(
            this.$axios.post(
              `${this.$dataApi.realEstateClose}/${this.realEstate.id}`
            ),
            (e) => {
              this.realEstate.status = e.data.status;
              this.$refs['form'].instance.repaint()
              this.$awn.success();
            },
            (e) => {
