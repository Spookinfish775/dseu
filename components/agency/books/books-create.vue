<template>
	<div>
		<BaseToolbar :canSave="canCreate" @save="saveBooks" />
		<DxForm ref="form" label-location="top" :form-data.sync="books">
			...
			<template #organizationSelectBox>
				<OrganizationSelectBox
					...
					:filter="['organizationType', '=', OrganizationType.Branch]"
...
import { Books } from "~/infrastructure/classes/Books";
...
export default Vue.extend({
	...
	methods: {
		...
		saveBooks() {
			...
			if (result.isValid) {
				this.$awn.asyncBlock(
					this.$axios.post(this.$dataApi.books, this.books),
					e => {
						this.$awn.success();
