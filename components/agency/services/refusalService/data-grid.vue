	...
	<div>
		...
		>
			...
			<DxExport
				...
				:file-name="$t('navigation.agency.refusalServiceTitle')"
			/>
			<DxStateStoring
				:enabled="true"
				type="localStorage"
				storage-key="refusalService"
			/>
			<DxEditing
				:allow-adding="false"
			...
			<DxColumn
				...
				data-type="number"
				:caption="$t('labels.refusalType')"
			>
				<DxLookup
					value-expr="id"
					display-expr="name"
					:data-source="refusalTypeDataSource"
				/>
			</DxColumn>
			<DxColumn
...
export default Vue.extend({
	...
	data() {
		return {
			dataSource: new DataSource({
				store: this.$dxStore({
					key: "id",
					loadUrl: this.$dataApi.services.refusalService
				})
			}),
			...
			refusalTypeDataSource: RefusalTypes(this)
		};
	},
	computed: {
		canCreate() {
			...
			return [
				{
					...
					onClick: e => {
						this.$router.push(
							`/agency/services/refusalService/${e.row.data.id}`
						);
					}
				}
			];
	...
	methods: {
		rowDblClick(e) {
			this.$router.push(`/agency/services/refusalService/${e.data.id}`);
		},
		toolbarPreparing(e) {
			e.toolbarOptions.items = [
				{
					...
					onClick: () => {
						this.$router.push(`/agency/services/refusalService/create`);
					}
				},
				{
					location: "after",
