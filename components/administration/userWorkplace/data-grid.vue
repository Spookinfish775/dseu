	...
	<div>
		...
		>
			...
			<DxExport
				...
				:file-name="$t('navigation.administration.userWorkplaceTitle')"
			/>
			...
			<DxColumn
				...
				data-type="boolean"
				:caption="$t('labels.mainWorkPlace')"
			/>
...
export default Vue.extend({
	...
	data() {
		return {
			dataSource: new DataSource({
				store: this.$dxStore({
					key: "id",
					loadUrl: this.$dataApi.userWorkplace
				})
			}),
			...
			return [
				{
					...
					icon: "info",
					onClick: e => {
						this.$router.push(`/administration/userWorkplace/${e.row.data.id}`);
					}
				}
			];
		}
	...
	methods: {
		rowDblClick(e) {
			this.$router.push(`/administration/userWorkplace/${e.data.id}`);
		},
		toolbarPreparing(e) {
			e.toolbarOptions.items = [
				{
					...
					onClick: () => {
						this.$router.push(`/administration/userWorkplace/create`);
					}
				},
				{
					location: "after",
