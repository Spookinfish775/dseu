<template>
	<div>
		...
		/>
		<BasePopup
			ref="realEstateMissionPopup"
			width="70vw"
			height="70vh"
			:show-title="true"
			:title="$t('labels.realEstateMission')"
		>
			<Card
				v-if="isCard"
				:data="selectedValue"
			...
			/>
			<CreateCard v-else @successedSaved="successedSaved" />
		</BasePopup>
	</div>
</template>
...
export default Vue.extend({
					...
					options: {
						...
						onClick: () => {
							if (this.valueExpr == "id") {
								this.$awn.asyncBlock(
									this.$axios.get(
										`${this.$dataApi.realEstateMission}/${this.value}`
									),
									e => {
										this.selectedValue = e.data;
										this.isCard = true;
										this.$refs["realEstateMissionPopup"].open();
									},
									e => {
										this.$awn.alert();
									}
	...
	methods: {
		async successedDeleted() {
			this.$refs["realEstateMissionPopup"].close();
