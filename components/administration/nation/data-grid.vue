	...
	<div>
		...
		>
			...
			<DxExport
				...
				:file-name="$t('navigation.administration.nationTitle')"
			/>
			<DxStateStoring
				:enabled="true"
				type="localStorage"
...
export default Vue.extend({
		...
		return {
			dataSource: new DataSource({
				store: this.$dxStore({
					...
					insertUrl: this.$dataApi.nation,
					removeUrl: this.$dataApi.nation,
					updateUrl: this.$dataApi.nation
				})
			}),
	...
	computed: {
		canCreate() {
			let permission: number = this.$store.getters["user/claims"]["Nation"];
