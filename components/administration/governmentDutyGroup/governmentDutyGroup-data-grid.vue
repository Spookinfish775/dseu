	...
	<div>
		...
		>
			...
			<DxExport
				...
				:file-name="$t('navigation.administration.governmentDutyGroupTitle')"
			/>
			<DxStateStoring
				:enabled="true"
				type="localStorage"
				storage-key="governmentDutyGroup"
			/>
			<DxEditing
				:allow-adding="canCreate"
				:allow-deleting="canUpdate"
				:allow-updating="fullAccess"
				:useIcons="true"
				mode="row"
			/>
		...
		return {
			dataSource: new DataSource({
				store: this.$dxStore({
					...
					insertUrl: this.$dataApi.governmentDutyGroup,
					removeUrl: this.$dataApi.governmentDutyGroup,
					updateUrl: this.$dataApi.governmentDutyGroup
				})
			}),
